...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	          lr_decay=1.0, lr_decay_every=1000, show_every=50, verbose=False):
	"""
	...
	for epoch in xrange(max_epochs):
		...
		for iter in xrange(iter_start, iter_end):
			...
			#############################################################################
			loss = None
			forward_data = model.forward(data_batch)
			loss = loss_func.forward(forward_data, labels_batch)
		...
		train_acc = compute_acc(model, data_train, labels_train, num_samples=490)
