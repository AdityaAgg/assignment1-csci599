...
class fc(object):
	...
	def forward(self, feat):
		...
		#############################################################################       
		#print feat
		input_reshape = feat.reshape(feat.shape[0], np.prod(feat.shape[1:]))
		#print input_reshape
		output = np.dot(input_reshape, self.params[self.w_name])
		local_b = self.params[self.b_name]
		output += local_b;        
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
	...
	def backward(self, dprev):
		...
		# corresponding name.                                                       # 
		#############################################################################
		error_transpose = dprev.transpose()
		prev_input = feat.reshape(feat.shape[0], np.prod(feat.shape[1:]));
		self.grads[self.w_name] = np.dot(error_transpose, prev_input).transpose()
		dfeat = np.dot(dprev, self.params[self.w_name].transpose())
		dfeat = dfeat.reshape((dfeat.shape[0],) + feat.shape[1:])
...
class dropout(object):
	...
	def forward(self, feat, is_Training=True):
		...
		output = feat;
		if is_Training:
			dropped = np.random.random(feat.shape) > self.p
...
class cross_entropy(object):
	...
	def forward(self, feat, label):
		...
		#############################################################################
		label_binary = np.zeros(scores.shape)
		label_binary[np.arange(scores.shape[0]), label] = 1;
		loss = sum(sum(np.log(scores) * label_binary)*-1)
		if self.dim_average:        
	...
	def backward(self):
		...
		#############################################################################
		label_binary = np.zeros(dLoss.shape)
		label_binary[np.arange(dLoss.shape[0]), self.label] = 1;
		#print dLoss
		#print label_binary
		dLoss = (dLoss - (label_binary))
		if self.dim_average:
...
def softmax(feat):
	...
	#############################################################################
	scores = np.exp(feat).transpose()/np.exp(feat).sum(axis=1).transpose()
